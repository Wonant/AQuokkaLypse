...
public class RopeBridge extends ObstacleGroup {
    ...
    protected boolean createJoints(World world) {
        ...
        for (int ii = 0; ii < sprites.size-1; ii++) {
            ...
            Obstacle midLink = sprites.get(ii).getObstacle();
            Obstacle midNextLink = sprites.get(ii+1).getObstacle();

            //RevoluteJointDef jointDefMid = new RevoluteJointDef();
            jointDef.bodyA = midLink.getBody();
            jointDef.bodyB = midNextLink.getBody();

            Vector2 anchor3 = new Vector2(linksize / 2, 0);
            jointDef.localAnchorA.set(anchor3);
            jointDef.localAnchorB.set(anchor2);
